...
class HybridHeadAttention(nn.Module):
    """
    Per-head hybrid attention:
    - If rank[head] < THRESHOLD → use PnP Nystrom for that head
    - If rank[head] >= THRESHOLD → use original softmax for that head
    """
    def __init__(self, dim, num_heads, head_ranks, num_landmarks=64, pinv_iters=6):
        super().__init__()
    ...
    def forward(self, x, attn_mask=None):
        ...
        for h in range(self.num_heads):
            ...
            if rank < THRESHOLD:
                # --- PnP Nystrom for this head ---
                q_m = landmark_pool(q_h, self.num_landmarks)
                k_m = landmark_pool(k_h, self.num_landmarks)
...
for i, block in enumerate(hybrid_model.blocks):
    ...
    assignment = ["P" if rank_matrix[i][h] < THRESHOLD else "S" for h in range(12)]
    pnp_count  = assignment.count("P")
    row = f"L{i:<5} " + " ".join([f"{a:<5}" for a in assignment]) + f"  ({pnp_count}/12 PnP)"
    print(row)
...
print_predictions("Hybrid Model - Top 5 Predictions", hybrid_probs, hybrid_indices, info)

...
parser.add_argument('--pnp-layers', default=None, type=str,
                    help='Comma-separated layer indices to replace with PnP attention e.g. "6,7,8,9,10,11"')
...
parser.add_argument('--dropin-og-layers', default=None, type=str,
                    help='Comma-separated layer indices to replace with TimmAttentionDropinOG')
...
def validate(args):
    ...
    if args.pnp_layers:
      layers = list(map(int, args.pnp_layers.split(',')))
      model = replace_attention_layers(
          model,
          layers_to_replace=layers,
          num_landmarks=args.pnp_landmarks,
          pinv_iters=args.pnp_iters,
      )
      _logger.info(f'Replaced attention layers {layers} with PnPNystromAttention_Sir_Stable '
                   f'(landmarks={args.pnp_landmarks}, iters={args.pnp_iters})')

    param_count = sum([m.numel() for m in model.parameters()])
    _logger.info('Model %s created, param count: %d' % (args.model, param_count))

    if args.dropin_og_layers:
          layers = list(map(int, args.dropin_og_layers.split(',')))
          model = replace_attention_layers_dropin_og(model, layers_to_replace=layers)
          _logger.info(f'Replaced attention layers {layers} with TimmAttentionDropinOG (sanity check)')

...
class AllPnPAdaptiveMAttention(nn.Module):
    """
    All heads use PnP Nystrom, but each head uses its own rank as M:
    - Every head → PnP with M = rank[head] landmarks
    """
    ...
    def forward(self, x, attn_mask=None):
        ...
        for h in range(self.num_heads):
            ...
            num_landmarks = max(1, rank)  # M = this head's rank

            # --- PnP Nystrom for ALL heads ---
...
# ============================================================
print("\n>>> STEP 3: Building all-PnP adaptive-M model...")

all_pnp_model = timm.create_model('vit_base_patch16_224', pretrained=True)
all_pnp_model.eval().to(DEVICE)

# Print M value used per head
print("\nLandmarks M used per head (all PnP):")
print(f"{'Layer':<6} " + " ".join([f"H{h:<6}" for h in range(12)]))
print("-" * 90)

for i, block in enumerate(all_pnp_model.blocks):
    old_attn   = block.attn
    dim        = old_attn.qkv.weight.shape[1]
    num_heads  = old_attn.num_heads
    head_ranks = [int(rank_matrix[i][h]) for h in range(num_heads)]

    new_attn = AllPnPAdaptiveMAttention(
        dim=dim,
        num_heads=num_heads,
    ...
    # Print M value per head
    row = f"L{i:<5} " + " ".join([f"{rank_matrix[i][h]:<7}" for h in range(12)])
    print(row)

print("\n>>> STEP 4: All-PnP adaptive-M model inference...")

with torch.no_grad():
    all_pnp_output = all_pnp_model(input_tensor)

all_pnp_probs, all_pnp_indices = get_top_k(all_pnp_output)
print_predictions("All-PnP Adaptive-M Model - Top 5 Predictions", all_pnp_probs, all_pnp_indices, info)
...
print("="*50)
print(f"\n{'Rank':<6} {'Softmax':^30} {'All-PnP Adaptive-M':^30}")
print("-"*66)
for i in range(TOP_K):
    s_desc = info.index_to_description(int(softmax_indices[i]))[:25]
    p_desc = info.index_to_description(int(all_pnp_indices[i]))[:25]
    s_prob = float(softmax_probs[i]) * 100
    p_prob = float(all_pnp_probs[i]) * 100
    print(f"{i+1:<6} {s_desc+f' ({s_prob:.1f}%)':^30} {p_desc+f' ({p_prob:.1f}%)':^30}")

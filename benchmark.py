...
from attn import replace_attention_layers, replace_attention_layers_dropin_og
...
# PnP attention args
parser.add_argument('--pnp-layers', default=None, type=str,
                    help='Comma-separated layer indices to replace with PnP Nystrom attention e.g. "6,7,8,9,10,11"')
parser.add_argument('--pnp-landmarks', default=64, type=int,
                    help='Number of landmarks for PnP Nystrom attention (default: 64)')
parser.add_argument('--pnp-iters', default=6, type=int,
                    help='Number of iterations for Moore-Penrose pseudo-inverse (default: 6)')
parser.add_argument('--dropin-og-layers', default=None, type=str,
                    help='Comma-separated layer indices to replace with TimmAttentionDropinOG (sanity check)')
...
class BenchmarkRunner:
    def __init__(
        ...
        # Replace attention layers with PnP if specified
        if pnp_layers is not None:
            layers = list(map(int, pnp_layers.split(',')))
            _logger.info(f'Replacing attention layers {layers} with PnP Nystrom attention '
                         f'(landmarks={pnp_landmarks}, iters={pnp_iters})')
            self.model = replace_attention_layers(
                self.model,
                layers_to_replace=layers,
                num_landmarks=pnp_landmarks,
                pinv_iters=pnp_iters,
            )
            
        if dropin_og_layers is not None:
            layers = list(map(int, dropin_og_layers.split(',')))
            _logger.info(f'Replacing attention layers {layers} with TimmAttentionDropinOG')
            self.model = replace_attention_layers_dropin_og(self.model, layers_to_replace=layers)
...
class InferenceBenchmarkRunner(BenchmarkRunner):
    ...
    def run(self):
        ...
        if 'cuda' in self.device and torch.cuda.is_available():
            ...
            _logger.info(f"Peak GPU memory: {peak_memory_mb:.2f} MB | "
                         f"Current allocated: {avg_memory_mb:.2f} MB")
